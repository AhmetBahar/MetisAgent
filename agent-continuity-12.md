...
Frontend ve backend arasındaki API çağrılarının senkronizasyonu
TaskRunnerAPI'nin doğru referansının sağlanması (ReferenceError çözüldü)
MCP mimarisine tamamen geçerken eski yapı ile uyumluluğun korunması
Context ve placeholder güncellemelerinin güvenilir şekilde sağlanması
Görevler arası bağımlılık kontrollerinin ID bazlı çalışacak şekilde düzenlenmesi

Bekleyen Görevler (Güncelleme)

Backend API servislerinin tamamlanması:

_evaluate_task_result fonksiyonunun MCPCoordinator'a eklenmesi
Context yönetimi için yeni endpoint'lerin app.py'ye eklenmesi


Frontend ve backend koordinasyonunun tamamlanması:

Paralel task çalıştırma için backend entegrasyonu
LLM değerlendirme sonuçlarının UI'da gösterilmesi iyileştirmeleri


Context yönetimi için ekstra özellikler:

Context değerlerinin manuel olarak düzenlenebilmesi
Context değerlerini gösterecek bir UI paneli eklenmesi
